...
/**
 * Compare specs objects for equality
 */
function specsMatch(specs1: Record<string, string>, specs2: Record<string, string>): boolean {
...
export function addItemToCart(cart: Cart, newItem: Omit<CartItem, 'id'>): Cart {
	// Find existing item by matching both SKU and specs
	const existingItemIndex = cart.items.findIndex(
		item => item.productSku === newItem.productSku && specsMatch(item.specs, newItem.specs),
	)

	let updatedItems: CartItem[]

	if (existingItemIndex >= 0) {
		// Item exists with same SKU and specs, update quantity
		updatedItems = cart.items.map((item, index) =>
			index === existingItemIndex ? { ...item, quantity: item.quantity + newItem.quantity } : item,
		)
	} else {
		// New item (different SKU or different specs), add to cart
