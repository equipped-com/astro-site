...
Create the complete multi-tenant database schema for Equipped platform. This schema supports:
- Multi-tenant architecture with Account (organization) scoping
- Person vs User distinction (employees vs platform users)
- Team-based resource ownership (orders, devices, billing)
- Consultant pattern (users accessing multiple accounts)
- Leasing and order management

## Acceptance Criteria

- [ ] `migrations/0001_initial.sql` created with complete schema
- [ ] All tables from PRD Database Schema section implemented
- [ ] Proper foreign key relationships established
- [ ] Indexes on frequently queried columns
- [ ] Drizzle ORM schema file mirrors SQL schema
...
Feature: Database Schema
  ...
  So that multi-tenant data is isolated and queryable

  Scenario: Create all tables successfully
    When I run the initial migration
    Then all tables should be created without errors
    And foreign key constraints should be valid

  Scenario: User can access multiple accounts
    Given a User exists with id "user_123"
    And an Account exists with id "acct_abc"
    And an Account exists with id "acct_xyz"
    When I create AccountAccess records linking the user to both accounts
    Then the user should have access to both accounts
    And each access record should have a different role

  Scenario: Person exists without platform access
    Given an Account exists with id "acct_abc"
    When I create a Person record without account_access_id
    Then the person should exist in the directory
    And the person should NOT have login capabilities
    And devices can still be assigned to this person

  Scenario: Resources are tenant-scoped
    Given Account "acct_abc" has devices
    And Account "acct_xyz" has devices
    When I query devices for "acct_abc"
    Then I should only see devices belonging to "acct_abc"
    And "acct_xyz" devices should not be visible
```
...
Create `src/db/schema.ts` with TypeScript types matching the SQL schema above.
...
- PRD.md Section: Database Schema
- PRD.md Section 16: Multi-Tenancy Architecture
- documentation/platform-leasing.md Section 12: Data Model
